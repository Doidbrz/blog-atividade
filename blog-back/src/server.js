...
app.post('/api/create-account', async (req, res) => {
    ...
    res.status(201).json({ message: 'Usuário criado com êxito!' });
...
app.post('/api/create-article', async (req, res) => {
    ...
    if (existingArticle) {
        return res.status(400).json({ message: 'Artigo ja existente!' });
    }
...
app.put('/api/articles/:name/upvote', async (req, res) => {
    ...
    if (article) {
        article.upVotes += 1;
        res.send(`O artigo ${name} tem ${article.upvotes} votos!`)
    }
    else {
        res.send("Artigo inexistente");
    }
});
